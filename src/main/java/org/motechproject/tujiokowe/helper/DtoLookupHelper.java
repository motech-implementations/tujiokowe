...
import org.motechproject.tujiokowe.domain.enums.DateFilter;
...
public final class DtoLookupHelper {
  ...
  public static GridSettings changeLookupForVisitReschedule(GridSettings settings)
      throws IOException {  //NO CHECKSTYLE CyclomaticComplexity
    ...
    switch (lookup) {
      ...
      case "Find By Visit Type Set And Planned Date":
        if (rangeMap != null && (StringUtils.isNotBlank(rangeMap.get("min")) || StringUtils
            .isNotBlank(rangeMap.get("max")))) {
      ...
      default:
        if (rangeMap != null && (StringUtils.isNotBlank(rangeMap.get("min")) || StringUtils
          ...
          fieldsMap.put(Visit.VISIT_PLANNED_DATE_PROPERTY_NAME, rangeMap);
          fieldsMap
              .put(Visit.VISIT_TYPE_PROPERTY_NAME, AVAILABLE_VISIT_TYPES_FOR_RESCHEDULE_SCREEN);
        } else {
          fieldsMap
              .put(Visit.VISIT_TYPE_PROPERTY_NAME, AVAILABLE_VISIT_TYPES_FOR_RESCHEDULE_SCREEN);
  ...
  public static GridSettings changeLookupAndOrderForFollowupsMissedClinicVisitsReport(
      GridSettings settings) throws IOException { //NO CHECKSTYLE CyclomaticComplexity
    ...
    if (StringUtils.isBlank(settings.getLookup())) {
      settings
          .setLookup("Find By Planned Date Less And Actual Date Eq And Subject Phone Number Eq");
      ...
      switch (settings.getLookup()) {
        ...
        default:
          fieldsMap
              .put(Visit.VISIT_PLANNED_DATE_PROPERTY_NAME, LocalDate.now().toString(formatter));
          fieldsMap.put(Visit.VISIT_DATE_PROPERTY_NAME, null);
  ...
  public static GridSettings changeLookupAndOrderForMandEMissedClinicVisitsReport(
      GridSettings settings) throws IOException { //NO CHECKSTYLE CyclomaticComplexity
    DateTimeFormatter formatter = DateTimeFormat.forPattern("yyyy-MM-dd");
    Map<String, String> fieldsMap = new HashMap<>();
    String newLookupName;
  ...
  public static GridSettings changeLookupAndOrderForOptsOutOfMotechMessagesReport(
  ...
  private static boolean checkAndUpdateDateRangeForFollowupsMissedClinicVisitsReport(
      Range<LocalDate> dateRange, GridSettings settings) {
    if (dateRange == null) {
      return false;
    }
    LocalDate maxDate = LocalDate.now().minusDays(1);
    if (dateRange.getMin() != null && dateRange.getMin().isAfter(maxDate)) {
      ...
      settings.setFields(
          setNewMaxDateInRangeFields(settings.getFields(), Visit.VISIT_PLANNED_DATE_PROPERTY_NAME,
              maxDate));
    }
    return true;
  }
  ...
  private static Object getObjectFromLookupFields(String lookupFields, String fieldName) {
    ...
    try {
      ...
      throw new TujiokoweLookupException("Invalid lookup params", e);
    }
  ...
  private static Range<LocalDate> getDateRangeFromLookupFields(String lookupFields,
      String dateName) {
    DateTimeFormatter formatter = DateTimeFormat.forPattern("yyyy-MM-dd");
    LocalDate min = null;
    LocalDate max = null;
    ...
    Map<String, String> rangeMap = (Map<String, String>) getObjectFromLookupFields(lookupFields,
        dateName);
  ...
  private static String setNewMaxDateInRangeFields(String lookupFields, String dateName,
      LocalDate date) {
    DateTimeFormatter formatter = DateTimeFormat.forPattern("yyyy-MM-dd");
    Map<String, Object> fieldsMap;
  ...
  private static Map<String, String> getDateRangeFromFilter(GridSettings settings) {
    ...
    if (DateFilter.DATE_RANGE.equals(dateFilter)) {
      ...
      rangeMap.put("min", dateRange.getMin().toString(TujiokoweConstants.SIMPLE_DATE_FORMAT));
      rangeMap.put("max", dateRange.getMax().toString(TujiokoweConstants.SIMPLE_DATE_FORMAT));
    }
